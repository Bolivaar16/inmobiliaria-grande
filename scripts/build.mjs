...
/* es-ES omits the separator on four digits ("2328"); Spanish readers expect "2.328". */
const nf = { es: new Intl.NumberFormat("es-ES", { maximumFractionDigits: 0, useGrouping: "always" }), en: new Intl.NumberFormat("en-GB", { maximumFractionDigits: 0 }) };
...
/* Card specs add the price per m² (sales only): the figure buyers use to compare. */
function cardSpecs(listing, lang) {
  const out = specs(listing, lang);
  const ppm2 = pricePerM2(listing);
  if (ppm2 != null) out.push(`${fmtInt(ppm2, lang)} ${i18n.ui[lang].per_m2}`);
  return out;
}
...
function card(listing, lang, root, opts = {}) {
  ...
  const sp = cardSpecs(listing, lang).map((s) => `<li>${s}</li>`).join("");
...
const FIG = {
  es: { built: "Superficie construida", useful: "Superficie útil", ppm2: "Precio por m²", rooms: "Dormitorios", baths: "Baños", cond: "Estado", plot: "Parcela", year: "Año", ref: "Referencia" },
  en: { built: "Built area", useful: "Usable area", ppm2: "Price per m²", rooms: "Bedrooms", baths: "Bathrooms", cond: "Condition", plot: "Plot", year: "Year built", ref: "Reference" },
};
function figuresHtml(listing, lang) {
  ...
  const ppm2 = pricePerM2(listing);
  if (ppm2 != null) out.push(item(L.ppm2, `${fmtInt(ppm2, lang)}\u00A0€/m²`));
