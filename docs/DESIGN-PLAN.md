...
2. Dirección: "Inmobiliaria Grande" / "Calle Recogidas 13, 1.º A" / "18008 Granada".
...
4. Dos columnas (60/40 desktop, apiladas en móvil, formulario primero en el DOM):
   ...
   - **Datos**: dirección completa, teléfonos, correo, y `.map-box`: caja de 4:3 con borde 1 px `--line`, fondo `--violet-100`, roseta central al 12 % de opacidad, dirección en grande y `a.btn--ghost` "Abrir en Google Maps" → `https://www.google.com/maps/search/?api=1&query=Calle+Recogidas+13,+18008+Granada` (`target="_blank" rel="noopener"`). **Ningún iframe.** Nota para el ejecutor: si más adelante se sustituye por una imagen estática, va en `assets/brand/map-recogidas.png` con `alt="Plano de la zona de Calle Recogidas 13, Granada"`; hoy no existe, se queda la caja.
...
| `contacto.html` | Contacto \| Inmobiliaria Grande, Granada | Oficina en Calle Recogidas 13, 1.º A, 18008 Granada. Teléfono 958 25 24 61 y WhatsApp. Escríbenos y te contestamos lo antes posible. |
...
| `aviso-legal.html` | Aviso legal \| Inmobiliaria Grande, Granada | Aviso legal y condiciones de uso del sitio web de Inmobiliaria Grande, Calle Recogidas 13, 1.º A, 18008 Granada. |
...
**JSON-LD.** En `index.html`, un `RealEstateAgent`: `name`, `url`, `image` (logo absoluto), `telephone: "+34958252461"`, `email`, `address` (`PostalAddress`: `Calle Recogidas 13, 1.º A` / `Granada` / `18008` / `ES`), `foundingDate: "1970"`, `areaServed: {"@type":"AdministrativeArea","name":"Granada"}` y `openingHours` (dos tramos: la ficha de Google confirmó la jornada partida, 2026-09-03). **Sin** `aggregateRating`: la valoración se publica como texto en el bloque de reseñas, no como marcado.
